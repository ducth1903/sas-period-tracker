...
const DateCircle = ({ inText, outerRotate, innerRotate, currentDay, periodDays, dayStatus }) => {
    return (
        <View
        ...
        >
            ...
            <View className={`-m-4 ${currentDay === inText ? "bg-salmon" : periodDays.includes(inText) ? "bg-salmon/50" : "bg-teal/20"} flex-none w-6 aspect-square rounded-full items-center justify-center`}
                style={innerRotate}>
                <Text onPress={()=>dayStatus(inText)} className={`${inText === currentDay ? "text-slate-50" : "text-teal"} text-s font-medium`}>{inText}</Text>
            </View>
        </View>
    )
}
