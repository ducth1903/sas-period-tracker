...
const ResourceHomeScreen = ({ navigation, props }) => {
  ...
  function findFavorites(resourceMap, lang, favorites) {
    return resourceMap
      ...
      .flat(2)
      .filter((article) => article); // filter out null values
  }

  async function initFavoritedResources(resourceMap) {
    try {
      let favoritedResourcesResp = await AsyncStorage.getItem(
        "favoritedResources"
      );
      if (!favoritedResourcesResp) {
        ...
        await AsyncStorage.setItem(
          "favoritedResources",
          JSON.stringify(userKeyedFavoritedResources)
        ); // init empty saved resources
        favoritedResourcesResp = await AsyncStorage.getItem(
          "favoritedResources"
        );
      }
      ...
      if (!parsedResp[userId]) {
        ...
        parsedResp = JSON.parse(
          await AsyncStorage.getItem("favoritedResources")
        );
      }
      ...
      console.log(
        `[ResourceHomeScreen] error initializing saved resources: ${error}`
      );
    }
  }
  ...
  async function fetchAllResources(attempts = 1) {
    setIsLoading(true);
    console.log(
      `[ResourceHomeScreen] fetching all resources attempt ${attempts}`
    );

    //Fetch content
    let content;
    const contentResp = await fetch(`${API_URL}/resources/content`, {
      method: "GET",
    });
    if (contentResp.ok) {
    ...
    //Fetch metadata
    let metadata;
    const metadataResp = await fetch(`${API_URL}/resources/metadata`, {
      method: "GET",
    });
    if (metadataResp.ok) {
    ...
    await Promise.all(
      content.map(async (item, index, arr) => {
        const resourceText = await fetchSingleResource(item["resource_url"]);
        if (resourceText === null) {
          console.log(
            `Skipping resource due to failed fetch for URL: ${item["resource_url"]}`
          );
    ...
    content.forEach((item) => {
      ...
      const topicObject = resourceMap.find(
        (obj) =>
          obj[itemLanguage]["topicTitle"] ===
          topics_to_titles[itemLanguage][itemTopic]
      );

      if (!itemSection) {
        // is intro file, intro files don't have sections
        topicObject[itemLanguage]["introText"] = itemText;
        return;
      }

      // create new section object if it doesn't exist
      let sectionObject = topicObject[itemLanguage]["sections"].find(
        (section) =>
          section["sectionTitle"] ===
          sections_to_titles[itemLanguage][itemSection]
      );
      if (!sectionObject) {
        sectionObject = {
  ...
  async function fetchSingleResource(resource_url) {
    try {
      ...
      if (resp.ok) {
        ...
        console.error(
          `Fail to fetch the resource text content from the url: ${resource_url}. Status: ${resp.status}`
        );
        return null;
      }
    } catch (error) {
  ...
  function SectionItem({ item }) {
    ...
    let otherLanguages = Object.keys(i18n.translations).filter(
      (lang) => lang !== selectedSettingsLanguage
    );
    ...
    for (const lang in otherLanguages) {
      ...
      const matchingSection = item.parentTopicObject[otherLang].sections.find(
    ...
    return (
      ...
      >
        <ImageBackground
          source={{ uri: item["parentTopicObject"]["image"] }}
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {resourcesMap.map((ele, index) => {
          ...
          if (
            index >= 1 &&
            ele[selectedSettingsLanguage] &&
            ele[selectedSettingsLanguage]["sections"]
          ) {
            if (ele[selectedSettingsLanguage]["sections"].length > 0) {
              return (
                <View key={`${ele["en"]["topicTitle"]}-${index}`}>
                  <Text style={styles.subHeaderText}>
                    {ele[selectedSettingsLanguage]["topicTitle"]}
                  </Text>
                  <FlatList
                    horizontal
            ...
            ele[selectedSettingsLanguage] &&
            ele[selectedSettingsLanguage]["articles"]
          ) {
            return (
              <View key={`${ele["en"]["topicTitle"]}-${index}`}>
                <Text style={styles.subHeaderText}>
                ...
                {ele[selectedSettingsLanguage]["articles"].length > 0 ? (
                  <FlatList
                    ...
                    renderItem={({ item }) => (
                      <ArticleItem item={item} key={item.articleTitle} />
                    )}
                    showsHorizontalScrollIndicator={false}
                    style={{ marginBottom: -15 }}
                  ...
                  <View style={styles.articleBox}>
                    <View style={styles.darkness} />
                    <Text style={styles.articleBoxText}>
                      {i18n.t("education.noArticlesFavorited")}
                    </Text>
                  </View>
                )}
              </View>
